...
/*		문자열형 숫자를 4칙연산이 되는 숫자 데이터로 변환시키는 방법
 * 		2가지 method가 있다.    
...
* 		이 method는 데이터를 unboxing하는 절차가 추가된다
* 		일부에서는 성능상 문제가 있다고 하지만 지금은 크게 문제되지 않는다.
* 
* 문자열형 숫자를 실제 숫자로 바꿀 때는 type.value()method를
* 사용해도 큰 문제가 없다.
*/
		float flaotNum1=Float.parseFloat("30.0");
